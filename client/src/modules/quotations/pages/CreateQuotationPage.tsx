...
import { useCreateQuotationMutation, type QuotationItemInput } from '../hooks/useQuotations';
...
export const CreateQuotationPage: React.FC = () => {
  ...
  const [currency] = useState(workspace?.currency || 'USD');
  const [notes, setNotes] = useState('');
  const [terms, setTerms] = useState('Payment due within 30 days of proposal acceptance.');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!clientId) {
      setErrorMsg('Please select a client for this proposal');
      return;
    }

    if (items.length === 0) {
      setErrorMsg('Please add at least one item line to the quotation');
      return;
    }
    ...
    try {
      ...
      const qNum = res?.data?.quotationNumber || '';
      navigate('/quotations', { state: { message: `Quotation proposal ${qNum} generated successfully!` } });
    } catch (err: any) {
      const serverErrors = err.response?.data?.errors;
      if (Array.isArray(serverErrors) && serverErrors.length > 0) {
        setErrorMsg(`Validation failed: ${serverErrors.map((e: any) => `${e.field}: ${e.message}`).join(', ')}`);
      } else {
        setErrorMsg(err.response?.data?.message || 'Failed to generate quotation proposal.');
      }
    }
  };
  ...
  return (
    <div className="relative overflow-hidden space-y-6 max-w-5xl mx-auto pb-12">
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight font-heading text-gray-900 dark:text-white">
              Create New Quotation Proposal
            </h1>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
              Draft estimations, configure product line rates, and issue proposals to prospects
            </p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
        {/* Top Details Card (Client, Dates, Status) */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <div className="md:col-span-1 space-y-1.5">
              ...
              <div className="relative">
                ...
                >
                  <option value="" className="dark:bg-[#16151a] text-gray-400">Select a Client...</option>
                  {clients.map((c: any) => (
        ...
        <div className="p-6 rounded-[22px] border border-gray-200/80 dark:border-white/10 bg-white/70 dark:bg-[#121118]/70 backdrop-blur-xl shadow-sm space-y-4">
          <div className="flex items-center justify-between border-b border-gray-100 dark:border-white/10 pb-3">
            ...
            <button
              type="button"
              onClick={addItemRow}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-blue-50 dark:bg-blue-600/10 text-blue-600 dark:text-blue-400 font-bold text-xs hover:bg-blue-100 dark:hover:bg-blue-600/20 transition cursor-pointer"
            >
                ...
                {items.map((item, index) => {
                  ...
                  return (
                    <tr key={index} className="hover:bg-gray-50/50 dark:hover:bg-white/[0.02]">
                      ...
                      {/* QTY with restored spinners & dark mode arrows */}
                      <td className="py-2.5 px-3">
                        <input
                          type="number"
                          min="0.01"
                      ...
                      {/* RATE with restored spinners & dark mode arrows */}
                      <td className="py-2.5 px-3">
                        <input
                          type="number"
                          min="0"
        ...
        {/* Bottom Section: Notes & Live Grand Total Preview */}
          ...
          <div className="md:col-span-2 p-6 rounded-[22px] border border-gray-200/80 dark:border-white/10 bg-white/70 dark:bg-[#121118]/70 backdrop-blur-xl shadow-sm space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                placeholder="Scope of work details or thank you note..."
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                placeholder="Validity terms, milestone payments, or tax rules..."
                className="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#16151a] text-gray-900 dark:text-white text-xs focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
          </div>
