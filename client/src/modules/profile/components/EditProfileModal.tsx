...
export const EditProfileModal: React.FC<EditProfileModalProps> = ({
  ...
  const modalMarkup = (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-[#12101b] border border-white/10 rounded-3xl max-w-3xl w-full max-h-[90vh] overflow-y-auto custom-scrollbar shadow-2xl space-y-6 text-white p-6 sm:p-8 relative">
        {/* CLOSE BUTTON */}
        <button
          type="button"
          onClick={onClose}
        ...
        <div className="border-b border-white/10 pb-4 space-y-1">
          <div className="flex items-center gap-2 text-violet-400">
            ...
            <h2 className="text-xl font-black font-heading text-white">Edit Profile Details</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-white/10">
            <button
              type="button"
              onClick={onClose}
              className="py-2.5 px-5 rounded-full bg-transparent hover:bg-white/10 text-gray-400 hover:text-white text-xs font-semibold cursor-pointer transition"
            >
              Cancel
            </button>
            <button
